...
public class GrantCommand extends Command {
    ...
    public String usage(boolean isShort) {
        if (isShort) {
            return "+grant [userPing/ID] [nums] - Выдает привилегии пользователю. [nums] разделены точками с запятой";
        } else {
            StringBuilder builder = new StringBuilder("+grant [userPing/ID] [nums] - Выдает привилегии пользователю. [nums] разделены точками с запятой");
    ...
    public CommandExecutor executor() {
        return (context, args) -> {
            ...
            permission.addPermission(types);

            StringBuilder builder = new StringBuilder();
            if (!unparsed.isEmpty()) {
                builder.append("Не удалось выдать привилегии с номерами ");
            ...
            if (!types.isEmpty()) {
                builder.append("Выданы привилегии с номерами ");
